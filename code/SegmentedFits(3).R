# Testing other ways to fit piecewise lime
library(segmented)
library(tidyverse)
library(ggplot2)
library(tidyr)
...
# (3) since this is the third method of dealing with zeros/fitting piecewise line
write.csv(LogILIplus,"log_ILIplus(3).csv",row.names = FALSE)
...
for (n in StartYear:EndYear) {
  for (s in 1:S) {
    ...
    if(sum(PercentPosn$PERCENT.POSITIVE=="X")>5 | sum(PercentPosn$PERCENT.POSITIVE==0)>40) {
      ...
      # # plot residualts
      # png(paste("~/FluForecasting/FluForecasting/plots_residuals/",states[s],n,".png",sep = ""))
      # plot(week, LogIncidenceN-predict(segmented.fitN), main = sprintf("Flu incidence residuals in %s over the %i-%i season", states[s], n,n+1))
      # dev.off()
      ...
      # plot acf to check if autocorrelated
      png(paste("~/FluForecasting/FluForecasting/plots_acf/",states[s],n,".png",sep = ""))
      acf <- acf(LogIncidenceN-predict(segmented.fitN),plot = FALSE)
      plot(acf, main = paste("p-value",boxtest$p.value, "auto.arima", as.integer(arima$arma)))
      dev.off()
    }  
  }
}
